...
\includegraphics{www/co_dola_div_localgov_300_rgb.png}
...
```{r popage3}
if("pop" %in% unlist(outChk)) {
  popa2p <- filemat[18]  #age Plot Plot
...
```{r popage4}
if("pop" %in% unlist(outChk)) {
  popa1p <- filemat[21]  #age Plot Plot
...
```{r popage6}
if("pop" %in% unlist(outChk)) {
  popa3p <- filemat[26]  #age Plot Plot
...
```{r popage8}
if("pop" %in% unlist(outChk)) {
  popa4p <- filemat[29]  #age Plot Plot
...
if("popc" %in% unlist(outChk)) {
  ...
  popc2p <- filemat[35]  #Education Plot
...
```{r housing2}
if("housing" %in% unlist(outChk)) {
  poph1p <- filemat[43]  #Housing Forecast Plot 
...
```{r commute3}
if("comm" %in% unlist(outChk)) {
  liveText <- function(place) {
   OutText <- paste0("This table shows the top 10 places where people who live in ",place," work.")
   return(OutText)
}
  cat("\\newpage\n")
  cat(dget(filemat[58]))
  cat("\\FloatBarrier\n")
  cat(liveText(placename))
  cat("\\FloatBarrier\n")
...
```{r commute4}
if("comm" %in% unlist(outChk)) {
  workText <- function(place) {
   OutText <- paste0("This table shows the top 10 places where people who work in ",place," live.")
   return(OutText)
  }
...
```{r commute5}
if("comm" %in% unlist(outChk)) {
  commute2p <- filemat[62]  #Jobs Migration Plot 
...
```{r industry2}
if("emplind" %in% unlist(outChk)) {
  popei1p <- filemat[65]  #Jobs Plot Plot
...
```{r industry4}
if("emplind" %in% unlist(outChk)) {
  popei2p <- filemat[68]  #Jobs Plot Plot
...
```{r industry6}
if("emplind" %in% unlist(outChk)) {
  popei3p <- filemat[72]  #Jobs Plot Plot
...
```{r employment2}
if("emply" %in% unlist(outChk)) {
  popem1p <- filemat[77]   #JobsPopForecast
  include_graphics(popem1p)
}
```
...
```{r employment4}
if("emply" %in% unlist(outChk)) {
  popem2p <- filemat[80]   #Weekly Wages Plot
...
```{r employment6}
if("emply" %in% unlist(outChk)) {
  popem3p <- filemat[83]   #Residential LF
  include_graphics(popem3p)
}
```
